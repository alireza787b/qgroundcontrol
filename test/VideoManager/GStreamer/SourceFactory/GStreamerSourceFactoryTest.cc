...
QByteArray makeTestJpeg(int width = 4, int height = 4)
{
...
void GStreamerTest::_testWebSocketJpegReconnectOwnership()
{
    if (!gst_element_factory_find("appsrc") || !gst_element_factory_find("jpegparse")) {
        QSKIP("appsrc/jpegparse plugins unavailable");
    }
    ...
    ignoreLogMessage("Video.GStreamer.GstVideoReceiver", QtCriticalMsg,
                     QRegularExpression(QStringLiteral("GStreamer error from")));

    QWebSocketServer server(QStringLiteral("QGC WebSocket reconnect test"), QWebSocketServer::NonSecureMode);
    QVERIFY(server.listen(QHostAddress::LocalHost, 0));
    QSignalSpy connectionSpy(&server, &QWebSocketServer::newConnection);
    ...
    QCOMPARE(startSpy.constFirst().constFirst().toInt(), static_cast<int>(VideoReceiver::STATUS_OK));
    QTRY_VERIFY_WITH_TIMEOUT(connectionSpy.count() >= 1, TestTimeout::mediumMs());

    QWebSocket* firstPeer = server.nextPendingConnection();
    QVERIFY(firstPeer);
    const QByteArray jpeg = makeTestJpeg();
    QVERIFY(!jpeg.isEmpty());
    QCOMPARE(firstPeer->sendBinaryMessage(jpeg), static_cast<qint64>(jpeg.size()));
    (void) firstPeer->flush();
    QTRY_VERIFY_WITH_TIMEOUT(receiver._sourceFrameCount.load(std::memory_order_acquire) > 0, TestTimeout::mediumMs());

    firstPeer->abort();
    QTRY_VERIFY_WITH_TIMEOUT(receiver._reconnectAttempts.load(std::memory_order_acquire) >= 1, TestTimeout::mediumMs());
    QTRY_VERIFY_WITH_TIMEOUT(connectionSpy.count() >= 2, TestTimeout::mediumMs());
    QWebSocket* secondPeer = server.nextPendingConnection();
    QVERIFY(secondPeer);

    QSignalSpy connectionsAfterStop(&server, &QWebSocketServer::newConnection);
    receiver.stop();
    QVERIFY_SIGNAL_WAIT(stopSpy, TestTimeout::mediumMs());
    QVERIFY(!receiver._runRequested.load(std::memory_order_acquire));
    QVERIFY_NO_SIGNAL_WAIT(connectionsAfterStop, 1200);
}
